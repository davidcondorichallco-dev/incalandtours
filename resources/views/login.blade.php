...
<body class="login-page">
    ...
    <main class="login-main"><div class="login-box"><span class="step-mark">PORTAL INTERNO</span><h1>Bienvenido de nuevo</h1><p>Ingresa para gestionar las operaciones de tu sucursal.</p>
        <form method="POST" action="{{ route('login.attempt') }}" data-login-form data-retry-after="{{ (int) session('retry_after', 0) }}">@csrf
            <label>Correo o nombre de usuario<input type="text" name="email" value="{{ old('email') }}" autocomplete="username" required autofocus></label>
        ...
        <div class="login-links"><a href="{{ route('home') }}" class="secondary">← Volver al inicio</a><a href="{{ route('tourist.online') }}" class="login-public">¿Eres viajero? Reserva tu aventura aquí →</a></div>
    </div></main>
</body></html>
